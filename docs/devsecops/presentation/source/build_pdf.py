...
story += [P("7.4 Static-analysis delivery controls", "H2Black"), P("Static checks are exposed through simple Make commands so the instructor and the team can reproduce the same controls locally."), P("<font name='Courier'>make build<br/>make sast-sonar<br/>make sast-pitest<br/>make sast-dependency-check</font>"), P("The CI workflow runs SonarCloud as a separate job, runs Dependency-Check and PITest in the static-analysis job, uploads reports even when a scan fails, and caches the Dependency-Check NVD database. The first NVD database download is large; later runs reuse the GitHub Actions cache and retrieve only updates."), table([["Control", "Evidence and decision"], ["SonarCloud", "Quality gate passed; issues reviewed and classified."], ["JaCoCo", "Coverage generated and imported into SonarCloud."], ["PITest", "Mutation reports generated; surviving mutants become test-strength follow-up."], ["Dependency-Check", "NVD report generated; 11 unique API Gateway CVEs remain open after the Boot patch upgrade."], ["CI", "Separate jobs, secret-based credentials, report upload and cache reuse."]], [45 * mm, 127 * mm]), PageBreak()]
story += [P("7.5 Dependency-Check results", "H2Black"), P("The NVD update completed and the dependency scan generated HTML and JSON reports. The initial API Gateway report contained 43 unique CVE identifiers. After upgrading Spring Boot from 4.1.0 to 4.1.1 and rerunning the scan, the result dropped to 11 unique CVEs: 4 Critical, 5 High and 2 Medium. The previous Spring Framework, Spring Security, Log4j, PostgreSQL and Hibernate findings are no longer present."), Image(str(NVD_RUN), width=82 * mm, height=57 * mm), Image(str(DEPENDENCY_CHECK), width=82 * mm, height=69 * mm), P("The captured NVD run shows the completed update, while the Dependency-Check report shows the resulting severity distribution. The NIST source reference is retained as supporting context for the vulnerability database used by the scan.", "SmallGray"), Image(str(NIST_AUTHORITY), width=175 * mm, height=73 * mm), P("All remaining findings are associated with embedded Tomcat 11.0.24. They remain open rather than being suppressed because the current Spring Boot patch line does not yet manage the required fixed Tomcat version. The next maintenance action is to adopt a compatible Spring Boot patch release, then repeat the build and runtime validation.", "BodyBlack"), table([["Result", "Decision"], ["NVD data", "Successfully updated and reused from the local/CI cache."], ["Report formats", "HTML and JSON generated for the parent and service modules."], ["OSS Index", "Disabled because the unauthenticated endpoint returned HTTP 401."], ["Before upgrade", "43 unique API Gateway CVEs: 15 Critical, 13 High, 14 Medium, 1 Low."], ["After upgrade", "11 unique API Gateway CVEs: 4 Critical, 5 High, 2 Medium; all embedded Tomcat 11.0.24."], ["Evidence", "dependency-check-report.html and dependency-check-report.json under each module target directory."]], [43 * mm, 129 * mm]), PageBreak()]
story += [P("7.6 Workshop 2 completion status", "H2Black"), P("The static-analysis setup is operational and reproducible. SonarCloud with JaCoCo, PITest, Dependency-Check, Maven verification and CI report-upload controls have all been executed. The justified code remediation identified by SonarCloud was applied, and the shared Spring Boot patch upgrade reduced the dependency baseline from 43 to 11 API Gateway CVEs."), P("Workshop 2 is complete as a documented analysis and remediation exercise, with one explicit residual dependency risk: the 11 open findings on embedded Tomcat 11.0.24. They are recorded for the next compatible framework upgrade and are not hidden by suppression or by the scan threshold.", "BodyBlack"), PageBreak()]
